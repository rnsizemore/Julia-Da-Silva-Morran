...
nscens = 3
pnd = 0.001         # prob. of x-chromosome nondisjunction
...
for scen in 1:3
    ...
    for rep = 1:nreps
        ...
        for gen = 1:ngens # loop through generations
            ...
            for i = 1:2:N
                ...
                if rand() < (freqm)       # freqm chance for outcrossing, otherwise selfing
                    ...
                    while m_sample == 0 # sample random male
                        ...
                        tempindex = rand(1:N)
                        if(indiv_vector[Int(ceil(tempindex/2))] == 1)
                            m_sample = tempindex
                        end
                    ...
                    for j = 1:L-1
                        if(rand() <= r) # 0.5 chance at each locus to swap between pairs
                            if h_sample % 2 == 0    # swap with index before or after h_sample depending on which in individual was sampled
                                hostsnew[i, j+1:L] = hosts[h_sample - 1, j+1:L]
                            else
                                hostsnew[i, j+1:L] = hosts[h_sample + 1, j+1:L]
                            end
                            hostsnew[i + 1, j+1:L] = hosts[h_sample, j+1:L]
            ...
            genrecomb[gen+1, scen] += 2*sum(indiv_vector)/(N*nreps)         # add this generation's recomb allele freq. to genrecomb
